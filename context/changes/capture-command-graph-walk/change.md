...
Capture's GenerateReplyCommand stops routing phases itself (`_dispatch` in application/capture/commands/send_message.py picks DRAFTING via available_transitions) and walks the graph through the shared StateMachine.advance() introduced by llm-adapter-distill-live, so the command no longer knows capture's route. Carry-over caveats from that discover-contracts session: (1) not behaviour-neutral — today a turn never returns from drafting to conversing, while looping advance until refusal would take DRAFTING→CONVERSING when conversation_requested holds; the per-turn move budget must be decided explicitly; (2) capture streams to HTTP, so GraphDispatcher.run() -> bool does not fit as is — needs a streaming variant or a capture-specific walk; (3) depends on advance landing in llm-adapter-distill-live first.
