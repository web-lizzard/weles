...
- [x] 7.4 Add the `AC-10` and `AC-11` markers — `pyproject.toml` — 7c0b3ac
- [x] 7.5 `cd backend && uv run pytest tests/bdd -m "capture-flow" -v` and `uv run pytest` green — 7c0b3ac

#### Manual

- [x] 7.6 `cd backend && uv run pytest tests/bdd --collect-only` lists both new scenarios with no unmatched step — 7c0b3ac
