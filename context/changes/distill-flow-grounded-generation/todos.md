...
- [x] 5.1 Add `mark_ready`, `mark_failed` and `_ensure_generating` with unimplemented bodies — `domain/distill/note.py` — 3f6bbd4
- [x] 5.2 Rename `NoteRepository.add` to `save` and add `CardRepository` with `save` / `list_by_note` — `domain/distill/ports.py` — 3f6bbd4
- [x] 5.3 Carry the rename to the adapter and the one call site — `adapters/out/in_memory/distill/note_repository.py`, `application/distill/commands/save_note.py` — 3f6bbd4
- [x] 5.4 Add the `InMemoryCardRepository` skeleton with `save`, `list_by_note`, `snapshot`, `restore` — `adapters/out/in_memory/distill/card_repository.py` — 3f6bbd4
- [x] 5.5 `cd backend && uv run ruff check src` and `uv run basedpyright` clean, `tests/unit/distill/test_save_note_command.py` still green — 3f6bbd4

#### Manual

- [x] 5.6 Grep `src` for `notes.add` and confirm no call site remains — 3f6bbd4

### Phase 6: Note transitions and repository ports — behavior

#### Tests

- [x] tests generated — 66b8300
...
- [x] 7.1 Add `CardProposal` carrying raw `front` / `back` / `quote` — `application/distill/value_objects.py` — 9f6581b
- [x] 7.2 Add the `CardGeneration` and `NoteDocumentParser` ports and `UnitOfWork.cards` — `application/distill/ports.py` — 9f6581b
- [x] 7.3 Bring the card repository under snapshot and restore — `adapters/out/in_memory/distill/unit_of_work.py` — 9f6581b
- [x] 7.4 Add the parser and generation adapter skeletons — `adapters/out/in_memory/distill/note_document_parser.py`, `adapters/out/in_memory/distill/card_generation.py` — 9f6581b
- [x] 7.5 `cd backend && uv run ruff check src` and `uv run basedpyright` clean, `uv run pytest` green — 9f6581b

#### Manual

- [x] 7.6 Import both ports and the extended `UnitOfWork` in a one-liner — 9f6581b
...
- [x] 8.8 R3-F2 test_note_document_parser.py duplicates the contract suite against the concrete adapter — 44ae651
- [x] 8.9 R3-F3 test_note_document_parser.py is not named in any phase's Changes Required — 44ae651
...
- [x] 9.1 Implement block-derived proposals, skipping any block whose remainder is empty — `adapters/out/in_memory/distill/card_generation.py` — 7b48bcf
- [x] 9.2 Append the one fabricated proposal with its sentinel quote — `adapters/out/in_memory/distill/card_generation.py` — 7b48bcf
- [x] 9.3 Write the generation contract suite incl. determinism and the single unresolvable proposal — `tests/unit/distill/contracts/test_card_generation_contract.py` — 7b48bcf
- [x] 9.4 `cd backend && uv run pytest` green — 7b48bcf

#### Manual

- [x] 9.5 Run the adapter on a two-block note in a one-liner and read the proposals — 7b48bcf
...
- [x] 12.1 Implement validate-via-`NoteSavedPayload` and delegate to the command — `adapters/out/worker/handlers/flashcard_gen.py` — 4ddbc10
- [x] 12.2 Implement the malformed-payload path: log and return without raising — `adapters/out/worker/handlers/flashcard_gen.py` — 4ddbc10
- [x] 12.3 Write the handler tests incl. the malformed-payload case — `tests/unit/distill/test_flashcard_gen_handler.py` — 4ddbc10
- [x] 12.4 `cd backend && uv run pytest` green — 4ddbc10

#### Manual

- [x] 12.5 `cd backend && uv run pytest tests/unit/distill -v` — 4ddbc10

### Phase 13: Composition

#### Automated

- [x] 13.1 Add `card_front_max` and `card_back_max` — `config/settings.py` — 00069cd
- [x] 13.2 Wire the card repository, parser, generation adapter, `CardFactory` and the extended distill unit of work — `adapters/compose.py` — 00069cd
- [x] 13.3 Register `FlashcardGenHandler` after `SaveNoteHandler` in the `OutboxWorker` handler list — `adapters/compose.py` — 00069cd
- [x] 13.4 Add the five new codes to `EXCEPTION_STATUS_MAP` — `adapters/http/errors.py` — 00069cd
- [x] 13.5 `cd backend && uv run pytest` green incl. the exhaustiveness test, `uv run ruff check src` and `uv run basedpyright` clean — 00069cd
