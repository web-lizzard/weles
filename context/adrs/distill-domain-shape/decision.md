...
### Card quality is a domain policy

What counts as a *good* card is domain knowledge, not orchestration, so it lives in `domain/distill/` as an explicit policy object rather than in a service or a config read.
...
- Minting a card now needs a policy in hand, so every construction site and every test that builds a card must supply one. That is the price of the rule being explicit and configurable rather than a constant compiled into a value object.
