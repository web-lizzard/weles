...
- **One value object, `CardSide`, used for both sides.** `domain/capture/value_objects.py` wraps every meaningful string this way — frozen, `value: str`, strip on the way in, its own `CoreException` on violation — and `Note.content` is a `NoteContent`, not a `str`, so bare strings here would be the exception rather than the rule. **`CardSide` enforces only what makes a side a side**: stripped, non-empty, and under an absolute constant bound above which the input is runaway model output rather than a card. The two sides share that rule, so they share a type; the asymmetry between them is real but lives in the policy below, which names it more precisely than two hardcoded constants could.
...
Resolution needs the parser, which is an adapter, so the `Card` aggregate cannot self-validate. **The application service resolves what the domain cannot reach; the domain judges what it resolved.** The service puts each proposal's quote through the parser port and hands the outcome to the domain, which mints either a live card or a discarded one — the same shape `capture-flow-domain-shape` used for vocabulary reconciliation, where the aggregates never see candidate strings or similarity scores, only already-resolved objects.

### Minting a card: a domain factory over policies

What counts as a *good* card is domain knowledge, not orchestration, so it lives in `domain/distill/` as explicit policy objects rather than in a service or a config read.

**`CardLengthPolicy`** — a frozen domain value object carrying `front_max` and `back_max`. A front should be short enough to answer at a glance and a back may run longer; the asymmetry is stated here, in the one place it varies, instead of being spread across two hardcoded constants. The composition root builds the policy from configuration and injects it; the domain defines the rule and never reads configuration itself, so `hexagonal-arch-shape`'s layering holds.

This is what makes the bound safely configurable at all. **A policy applies when a card is minted, not when it is reconstituted**, so tightening a bound can never make a stored card unloadable — which is exactly what would happen if the same number lived in `CardSide`'s constructor and a repository tried to rebuild a row written under a looser setting.

The split is therefore: `CardSide` says what a card side **is**, `CardLengthPolicy` says what a card side **should be**. A violation of the first is a construction error; a violation of the second mints a card discarded for `oversized`, so the proposal is recorded rather than destroyed. That matters because verbosity is the rule a language model will break routinely — enforcing brevity at construction would make the un-audited path the common one.

**`CardFactory`** is the single door a proposal passes through to become a card. It is a domain object, constructed once at composition with its policies, and it takes already-resolved input — the application service has made the parser call — returning either a live card or a discarded one. No call site threads a policy, and a second policy joins the collection later without touching `Card`, the service, or any handler.

Two rules make it deterministic:

- **Grounding is evaluated first, and it is not a policy.** It is the domain's one non-negotiable invariant, so it stays out of the configurable collection where it would read as one option among several. A fabricated card must be reported as fabricated even when it is also verbose, or the "the model invents" signal hides behind the "the model rambles" one.
- **The first violation wins.** A card carries one `Discard`, so the factory stops at the first rule broken, in declared order, and puts the specifics in `detail`.
...
`DiscardReason` has three values and all are settable today:

- **`ungrounded`** — the anchor did not resolve, so the note does not support the card.
- **`oversized`** — a side breached `CardLengthPolicy`. Because the bound is configurable, `detail` records **which side and which bound was breached**; without that a discard rate would be a number with no referent, since the same card would be live under a different policy.
...
The distinctions are load-bearing rather than decorative, because each feeds a different correction. A rising `ungrounded` rate says the generator is fabricating and the prompt or the port is wrong. A rising `oversized` rate says it is honest but verbose, which is a prompt fix or a policy that is set too tight. A rising `user_audit` rate says it is grounded and concise but producing cards the person does not want, and `overview-thougts` keeps exactly that signal so a later generation avoids near-duplicates of what was already turned down. An undifferentiated "removed" would preserve the row and destroy the only thing it is for.
...
- A proposal with an empty side, a side past the absolute bound, or two identical sides still never becomes a card and leaves no discarded row. Moving brevity into the policy takes the common case out of that hole, but the hole remains for structurally invalid proposals, which are visible only in whatever the generation adapter logs.
- The card set a note yields now depends on a configured value. Two installations, or one installation before and after a settings change, produce different cards from the same note, and only `detail` makes the discards comparable across them. Nothing recomputes past discards when the policy changes, so the history is a record of decisions taken under bounds that may no longer hold.
- The factory is a convention, not a lock. `Card` is a `BaseModel` and its constructor stays open, so nothing stops a handler or a test building a card directly and bypassing every policy. The same class of hazard as the boundary rules: it holds as far as review and tests reach, and no further.
- Only one reason is recorded per discard. A proposal that is both ungrounded and oversized reports the first rule broken, so the audit under-counts co-occurring problems and a discard-rate breakdown is a lower bound per reason rather than a partition.
- Policy order is now semantics. Reordering the collection silently changes which reason a card is discarded for, and therefore what the audit says about the generator, without changing which cards survive.
...
24. **Distinct `CardFront` and `CardBack` types**, each with its own length constant and error, exactly like the four capture wrappers whose separation is driven by differing limits. Raised by the user, who is right that a front should be short and a back may be long. Rejected once that difference moved into `CardLengthPolicy`: the two sides then share every *constructor* rule, so they share a type, and the asymmetry is named in the policy's own fields rather than implied by two class names. The residual gain — a swap failing to type-check — is small next to keyword construction and the contract test on the adapter that maps a proposal to a card, which is the only place a swap could occur.
...
26. **A tight length bound inside `CardSide`'s constructor**, enforcing brevity as a construction invariant. Rejected: it converts a quality judgement into a construction failure, which is the one path that destroys the proposal instead of recording it, and verbosity is precisely the rule a language model breaks routinely — so the un-audited path would become the common one. It also makes the bound unconfigurable in practice, since a card stored under a looser value could no longer be reconstituted.
27. **A factory that reads configuration to build the value object** with a per-installation limit. The user's proposal, and the right instinct about configurability. Rejected in that form on three counts: a bound that varies by configuration is not an invariant, so a stored card stops being loadable when the setting tightens; building a value object from configuration drags config into the domain layer, against `hexagonal-arch-shape`'s rule that domain imports nothing from application or adapters; and a single-user prototype has nobody to turn the knob. What the instinct wanted survives as `CardLengthPolicy`, where the domain owns the rule and the composition root supplies the numbers.
28. **The brevity rule as an application-layer policy**, checked by the service that resolves anchors. Recommended, and rejected by the user in favour of a domain policy. They are right: what makes a good flashcard is domain knowledge, and the service's job is only to reach through the parser port for what the domain cannot resolve itself.
29. **Threading the policy into each call site**, or a `Card.mint` classmethod taking it as an argument. The shape this decision was first written with. Rejected in favour of a factory: creation rules that need collaborators do not belong on the entity, and passing a policy at every construction site puts the same assembly work in every handler and every test.
30. **Grounding folded into the policy collection**, making the factory uniform — one list of rules, evaluated in order. Tempting, and structurally cleaner. Rejected: grounding is the only invariant this domain enforces and the collection is configurable, so sitting inside it would make the non-negotiable rule read as one option among several.
31. **Recording every violation on a discard** rather than the first. Preserves more of the signal. Rejected: `Discard` is one value object and a list of reasons complicates every read for an analysis nobody performs yet; `detail` carries specifics where they matter.
32. **Restoring a discarded card.** Raised against the terminal-discard rule: a person who deletes by mistake has no way back. Rejected for this version — restore has to be conditional on the reason, since an `ungrounded` card cannot return without breaking the invariant, and that is machinery for a mistake there is not yet a surface to make. Recorded as an accepted cost rather than resolved.
